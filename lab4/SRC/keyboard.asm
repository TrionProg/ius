...
;	SRC/keyboard.c:67: push_byte_to_buffer(&key_clicks, key_value[key]);
	...
	dec	sp
;	SRC/keyboard.c:69: if( mode==MODE_INT ) {
	mov	a,_MODE_INT
	cjne	a,_mode,00105$
;	SRC/keyboard.c:70: handler_int();
	ljmp	_handler_int
00105$:
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	SRC/keyboard.c:75: void scan_keyboard_int(){
;	-----------------------------------------
;	 function scan_keyboard_int
;	-----------------------------------------
_scan_keyboard_int:
;	SRC/keyboard.c:80: prescaler++;
	inc	_prescaler
;	SRC/keyboard.c:81: if( prescaler==2 ){
	mov	a,#0x02
...
00152$:
	ljmp	00124$
00153$:
;	SRC/keyboard.c:83: prescaler=0;
	mov	_prescaler,#0x00
;	SRC/keyboard.c:85: row_mask=scan_keyboard();
	lcall	_scan_keyboard
	mov	r2,dpl
;	SRC/keyboard.c:87: for( row=0;row<ROWS;row++ ){
	mov	r3,#0x00
00129$:
	mov	dptr,#_ROWS
	clr	a
	...
	ljmp	00132$
00154$:
;	SRC/keyboard.c:88: key=col*ROWS + row;
	mov	b,_col
	mov	a,r4
	mul	ab
	mov	r4,a
	...
	add	a,r4
	mov	r4,a
;	SRC/keyboard.c:90: if( row_mask>>row & 1 ){//клавиша нажата
	mov	b,r3
	inc	b
	mov	a,r2
	sjmp	00156$
...
00156$:
	djnz	b,00155$
	jnb	acc.0,00112$
;	SRC/keyboard.c:91: if( key_count[key]<KEY_COUNT_LIMIT ) {
	mov	a,r4
	add	a,#_key_count
	mov	r0,a
	mov	ar5,@r0
	...
	subb	a,r6
	jnc	00113$
;	SRC/keyboard.c:92: key_count[key]++;
	inc	r5
	mov	@r0,ar5
;	SRC/keyboard.c:94: if( key_count[key]>PRESS_COUNT && key_pressed_time[key]==0 ){//помечаем, как нажатую
	...
	mov	a,@r0
	jnz	00113$
;	SRC/keyboard.c:95: key_click(key);
	mov	dpl,r4
	push	ar2
	push	ar3
	push	ar4
	...
	pop	ar3
	pop	ar2
;	SRC/keyboard.c:96: key_pressed_time[key]=1;
	mov	a,r4
	add	a,#_key_pressed_time
	mov	r0,a
	mov	@r0,#0x01
	sjmp	00113$
00112$:
;	SRC/keyboard.c:100: if( key_count[key]>0 ) {
	mov	a,r4
	add	a,#_key_count
	mov	r0,a
	mov	a,@r0
	mov	r5,a
	jz	00113$
;	SRC/keyboard.c:101: key_count[key]--;
	dec	r5
	mov	@r0,ar5
;	SRC/keyboard.c:103: if( key_count[key]<RELEASE_COUNT && key_pressed_time[key]>0 ){//помечаем как отпущенную
	...
	mov	a,@r0
	jz	00113$
;	SRC/keyboard.c:104: key_pressed_time[key]=0;
	mov	@r0,#0x00
;	SRC/keyboard.c:105: key_repeat_time[key]=0;
	mov	a,r4
	add	a,#_key_repeat_time
	mov	r0,a
	mov	@r0,#0x00
00113$:
;	SRC/keyboard.c:110: if( key_pressed_time[key]>0 ){//Клавиша помечена как нажатая
	mov	a,r4
	add	a,#_key_pressed_time
	mov	r0,a
	mov	a,@r0
	mov	r5,a
	jz	00131$
;	SRC/keyboard.c:111: if( key_pressed_time[key]<KEY_START_REPEAT_DELAY ){
	...
	subb	a,r6
	jnc	00117$
;	SRC/keyboard.c:112: key_pressed_time[key]++;
	mov	a,r5
	inc	a
	mov	@r0,a
	sjmp	00131$
00117$:
;	SRC/keyboard.c:114: key_repeat_time[key]++;
	mov	a,r4
	add	a,#_key_repeat_time
	mov	r0,a
	mov	ar5,@r0
	inc	r5
	mov	@r0,ar5
;	SRC/keyboard.c:116: if( key_repeat_time[key]==KEY_REPEAT_DELAY ) {//повторяем каждые KEY_REPEAT_DELAY
	...
	mov	a,r5
	cjne	a,ar6,00131$
;	SRC/keyboard.c:117: key_click(key);
	mov	dpl,r4
	push	ar2
	push	ar3
	push	ar4
	...
	pop	ar3
	pop	ar2
;	SRC/keyboard.c:118: key_repeat_time[key]=0;
	mov	a,r4
	add	a,#_key_repeat_time
	mov	r0,a
	mov	@r0,#0x00
...
;	SRC/keyboard.c:87: for( row=0;row<ROWS;row++ ){
	...
	ljmp	00129$
00132$:
;	SRC/keyboard.c:124: col++;
	inc	_col
;	SRC/keyboard.c:126: if( col==COLS ){
	mov	dptr,#_COLS
	clr	a
	movc	a,@a+dptr
	mov	r2,a
	cjne	a,_col,00124$
;	SRC/keyboard.c:127: col=0;
	mov	_col,#0x00
00124$:
;	SRC/keyboard.c:131: if( speaker>0 ){//Спикер работает
	mov	a,_speaker
	jz	00133$
;	SRC/keyboard.c:132: speaker++;
	inc	_speaker
;	SRC/keyboard.c:134: if(speaker==SPEAKER_TIME){
	mov	dptr,#_SPEAKER_TIME
	clr	a
	movc	a,@a+dptr
	mov	r2,a
	cjne	a,_speaker,00133$
;	SRC/keyboard.c:135: disable_speaker();
	lcall	_disable_speaker
;	SRC/keyboard.c:136: speaker=0;//выключаем спикер
	mov	_speaker,#0x00
00133$:
	ret
	.area CSEG    (CODE)
